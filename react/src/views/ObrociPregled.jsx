...
export default function ObrociPregled() {
  ...
  try {
    const res = await axiosClient.get("/cilj"); // ovo ide prema /api/cilj
    console.log("Cilj response:", res.data);
    setCilj(res.data.cilj || null);
  } catch (err) {
        ...
        {dnevniObroci.length > 0 && (
          <div className="mb-8 bg-white rounded-lg shadow p-4">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={dnevniObroci.map(d => ({ datum: d.datum, kalorije: d.ukupno_kalorija }))}>
...
/>
        ...
        <div className="space-y-6 sm:space-y-0 sm:grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {dnevniObroci.map((d) => (
            <div
              key={d.datum}
              className="bg-white rounded-lg shadow p-4 hover:shadow-xl hover:scale-[1.02] transition-all duration-300 cursor-pointer"
              onClick={() => navigate(`/obroci/${d.datum}`)}
            >
                ...
                {d.obroci.map((obrok) => (
                  ...
                  >
                    ...
                    <p className="flex flex-wrap gap-2 mt-1">
                      {obrok.hrana.map((h) => (
                        <span
                          key={h.hrana?.id || h.custom_naziv || Math.random()}
