...
export default function Dashboard() {
  ...
  const [allHidriranost, setAllHidriranost] = useState([]);
  ...
  useEffect(() => {
    ...
    axiosClient.get("/hidriranost-danas")
      ...
      axiosClient.get("/hidriranost") 
      .then(res => setAllHidriranost(res.data))
      .catch(err => console.log(err));
  }, [user]);
            ...
            {lastCilj?.hidriranost && (
                <div className="w-40 h-40 mb-4 mx-auto">
                  <Chart
                    ...
                    data={[
                      ["Status", "Litraža"],
                      ["Popijeno", hidriranost?.ukupno ?? 0],
                      ["Preostalo do cilja", Math.max(lastCilj.hidriranost - (hidriranost?.ukupno ?? 0), 0)]
                    ]}
                    options={{
                      pieHole: 0.6,
                      pieSliceText: "none",
                      colors: ["#3B82F6", "#E5E7EB"], // plava za popijeno, siva za ostatak
                      legend: "none",
                      backgroundColor: "transparent",
                    }}
                  />
                  <div className="text-center -mt-24 text-lg font-semibold">
                    {Math.round(((hidriranost?.ukupno ?? 0) / lastCilj.hidriranost) * 100)}%
                  </div>
                </div>
              )}
               {lastCilj?.hidriranost && (
                  <p className="text-center text-gray-700 mb-4">
                    {hidriranost?.ukupno >= lastCilj.hidriranost
                      ? "Cilj je ispunjen! 🎉"
                      : `Još ${(lastCilj.hidriranost - (hidriranost?.ukupno ?? 0)).toFixed(1)} L do cilja`}
                  </p>
                )}
            ...
            {/* {allHidriranost.length > 0 && (
              <div className="mt-6 bg-blue-50 rounded-xl p-4 shadow M">
                <Chart
                  ...
                  data={[
                    ["Datum", "Popijeno (L)"],
                    ...allHidriranost.map(h => [formatDate(h.datum), h.ukupno])
                  ]}
                  options={{
                    title: "Napredak hidriranosti",
                    curveType: "function",
                    legend: { position: "bottom", alignment: "center" },
                    colors: ["#3B82F6"], // plava linija
                    pointSize: 5,
                    lineWidth: 3,
                    hAxis: {
                      title: "Datum",
          ...
          <div className="bg-surface rounded-xl p-6 shadow">
            ...
            {ciljevi.length === 0 ? (
              ...
              <>
                ...
                <Button onClick={() => setShowCiljevi(true)} className="text-textSecondary underline hover:opacity-80 mb-4">
                  Prikaži sve ciljeve
                </Button>
                ...
                <Chart
                  ...
                  data={[
                    ...
                    ...ciljevi.map(c => [formatDate(c.created_at), c.tezina, c.hidriranost])
                  ]}
                  options={{
          ...
          <div className="bg-surface rounded-xl p-6 shadow">
            ...
            {parametri.length === 0 ? (
              ...
              <>
                ...
                <Button onClick={() => setShowParametri(true)} className="text-textSecondary underline hover:opacity-80 mb-4">
                ...
                data={[
                  ["Datum", "Težina", "BMI"],
                  ...parametri.map(p => [formatDate(p.date), p.tezina, p.bmi])
                ]}
                options={{
